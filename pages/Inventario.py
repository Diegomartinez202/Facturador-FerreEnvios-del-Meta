...
def show():
    ...
    config_visual_maestra = {
        ...
        "descripcion": st.column_config.TextColumn(
            "Descripción del Producto", width="large"
        ),
        "cantidad": st.column_config.NumberColumn(
            "Stock Disponible", format="%d", width="medium"
        ),
        "precio": st.column_config.NumberColumn(
            "Precio Venta", format="$%d", width="medium"
        ),  # <-- Pegado
        "categoria": st.column_config.TextColumn("Categoría", width="medium"),
        "costo_proveedor": st.column_config.NumberColumn(
            "Costo Inversión", format="$%d", width="medium"
        ),  # <-- Pegado
        "tarifa_iva": st.column_config.NumberColumn(
            "IVA %", format="%d%%", width="small"
        ),  # <-- Pegado
        "codigo_unspsc": st.column_config.TextColumn("Cód. UNSPSC", width="medium"),
        "unidad_medida": st.column_config.TextColumn(
            "Unidad de Medida", width="medium"
        ),
    ...
    tab_lista, tab_carga, tab_editor = st.tabs(
        ["Lista de Precios", "📥 Cargar Excel/Word", "Editor Maestro"]
    )
    ...
    with tab_lista:
        ...
        if not df_prods.empty:
            ...
            busqueda = st.text_input(
                "Buscar por descripción, código o categoría", key="bus_inv_final"
            )
            ...
            if aplicar_iva:
                ...
                df_view["Subtotal"] = df_view["precio"] / (
                    1 + (df_view["Tarifa"] / 100)
                )
                df_view["Valor IVA"] = df_view["precio"] - df_view["Subtotal"]

                # Definimos las columnas incluyendo las de IVA
                cols_finales = [
                    "codigo",
                    "descripcion",
                    "cantidad",
                ...
                # Si no hay IVA, usamos el orden maestro estándar
                cols_finales = orden_maestro

            # --- 🛠️ LIMPIEZA RADICAL DE DECIMALES ---
            # Forzamos a que sean enteros para que NO existan los .000000
            columnas_num = ["cantidad", "precio", "costo_proveedor", "tarifa_iva"]
            if aplicar_iva:
                columnas_num.extend(["Subtotal", "Valor IVA"])

            for col in columnas_num:
                if col in df_view.columns:
                    df_view[col] = df_view[col].fillna(0).round(0).astype(int)

            # --- APLICACIÓN DEL FILTRO (MASK) ---
            # El filtro se aplica sobre df_view antes de mostrar
            mask = (
            ...
            df_display = df_view[mask][cols_finales].copy()

            label_precio = (
                "Precio Final (COP)"
                if not aplicar_iva
                else "Precio Total (Incluye IVA)"
            )

            # --- RENDERIZADO PROFESIONAL CON FORMATO COP Y ESTILOS ---
            config_visual_maestra["precio"] = st.column_config.NumberColumn(
                label_precio, format="$%d", width="medium"
            )
            ...
            # --- CONFIGURACIÓN DINÁMICA DEL LABEL ---
            label_precio = (
                "Total Neto (Sin IVA)"
                if not aplicar_iva
                else "Precio Total (Incluye IVA)"
            ...
            # 🚀 MEJORA: Sobrescribimos la configuración de la columna 'precio'
            # sin renombrar la columna en el DataFrame. Esto mantiene el $ funcional.
            config_visual_maestra["precio"] = st.column_config.NumberColumn(
                label_precio,
                format="$%d",
                width="medium",
                help="Valor final calculado según la liquidación de IVA",
            )
            ...
            st.info(
                "💡 Nota: El desglose detallado en el PDF solo se activa al procesar la factura."
            )
            st.caption(
                f"💡 Liquidación de IVA: {estado_iva}. Los productos en rojo tienen stock crítico (5 o menos)."
            )
        else:
    ...
    with tab_carga:
        ...
        st.info(
            "Requisito: El archivo debe tener las columnas: **Código, Descripción, Categoría, Cantidad, Precio**"
        )
        archivo = st.file_uploader(
            "Subir lista de precios (Excel o CSV)", type=["xlsx", "csv"]
        )
                ...
                if all(col in df_nuevo.columns for col in columnas_esperadas):
                    ...
                    if st.button("🚀 Actualizar Base de Datos Ahora"):
                        with st.spinner("Sincronizando inventario..."):
                            ...
                            df_nuevo["Cantidad"] = pd.to_numeric(
                                df_nuevo["Cantidad"], errors="coerce"
                            ).fillna(0)
                            df_nuevo["Precio"] = pd.to_numeric(
                                df_nuevo["Precio"], errors="coerce"
                            ).fillna(0)
                    ...
                    faltantes = [
                        c for c in columnas_esperadas if c not in df_nuevo.columns
                    ]
    ...
    with tab_editor:
        ...
        st.info(
            "Cambia precios, costos o nombres directamente. Usa la última fila para agregar nuevos."
        )
            ...
            if st.button("Guardar Cambios Permanentes"):
                ...
                with st.spinner("Sincronizando con el archivo de base de datos..."):
                    ...
                    if st.session_state.db.actualizar_inventario_completo(edited_df):
                        st.success(
                            "¡Base de datos actualizada físicamente! Los cambios persistirán."
                        )
                        st.balloons()
                        ...
                        st.error(
                            "No se pudo escribir en el archivo. Verifica que no esté abierto en otro programa."
                        )
...
if __name__ == "__main__":
    ...
    if "db" not in st.session_state:
        try:
            ...
            st.error(
                "❌ **Error Crítico:** No se encontró el archivo de base de datos en `modules/`."
            )
            st.info(
                "Verifica que el archivo se llame `database_manager.py` y exista `modules/__init__.py`."
            )
        except Exception as e:
