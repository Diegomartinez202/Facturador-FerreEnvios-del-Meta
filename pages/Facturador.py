...
def show():
    ...
    with st.expander(
        "Datos del Cliente", expanded=(sel_cliente == "-- Nuevo Cliente --")
    ):
        if sel_cliente != "-- Nuevo Cliente --":
            datos_c = df_clientes[df_clientes["nombre"] == sel_cliente].iloc[0]
            v_nit, v_tel, v_dir = (
                str(datos_c["nit_cedula"]),
                str(datos_c["telefono"]),
                str(datos_c["direccion"]),
            )
        ...
        c_nom = st.text_input(
            "Nombre / Razón Social",
            value="" if sel_cliente == "-- Nuevo Cliente --" else sel_cliente,
        )
    ...
    lista_opciones = [
        f"{p['descripcion']} ({p['codigo']}) - Disponibles: {p['cantidad']}"
    ...
    if seleccion != "-- Seleccione --":
        ...
        if not filtro.empty:
            ...
            with col_p3:
                ...
                if st.button(
                    "Agregar Ítem", disabled=boton_bloqueado, use_container_width=True
                ):
                    item = {
                        "cod": producto["codigo"],
        ...
        if st.session_state.carrito:
            ...
            for idx, item in enumerate(st.session_state.carrito):
                ...
                with r2:
                    ...
                    # Nombre y Categoría
                    st.markdown(
                        f"**{item['desc']}** <small style='color:gray'>({item.get('categoria', 'General')})</small>",
                        unsafe_allow_html=True,
                    )

                    # LÍNEA DE VALORES (Limpiamos el texto para que el $ se vea claro)
                    st.write(
                        f"CANT: {item['cant']} | PRECIO UNITARIO: {valor_unitario} | SUBTOTAL: {valor_subtotal}"
                    )
            ...
            if indices_a_borrar:
                st.warning(
                    f"{len(indices_a_borrar)} producto(s) seleccionados para eliminar."
                )
                ...
                with col_si:
                    if st.button(
                        ...
                        st.session_state.v_check = (
                            st.session_state.get("v_check", 0) + 1
                        )
                        st.rerun()
                with col_no:
            ...
            # 4. VACIADO TOTAL (Se muestra si NO hay nada seleccionado arriba)
                ...
                if not st.session_state.confirmar_vaciado:
                    ...
                    with st.container(border=True):
                        ...
                        with cn:
                            if st.button(
                                "NO, VOLVER", type="secondary", use_container_width=True
                            ):
                                st.session_state.confirmar_vaciado = False
                                st.rerun()
        else:
        ...
        # 2. Flete (Nuevo Campo)
        flete_ingresado = col_m2.number_input(
            "Valor del Flete ($)", min_value=0.0, step=1000.0, value=0.0
        )
    ...
    with st.expander("👁️ PREVISUALIZAR DATOS DEL DOCUMENTO", expanded=False):
        ...
        if "FACTURA" in tipo_doc.upper():
            ...
            st.warning(
                "Este documento es una **COTIZACIÓN** y no tiene validez legal ante la DIAN."
            )
        ...
        if st.session_state.carrito:
            ...
            with c_r2:
                # Usamos los valores calculados por utils.py para que coincidan con el PDF
                ...
                if fin["flete"] > 0:
                    st.write(f"**Flete (No Gravado):** ${fin['flete']:,.0f}")

                # --- 🚀 SOLUCIÓN AL NameError ---
                # Reemplazamos gran_total por fin['total'] que ya tiene el cálculo correcto
                st.subheader(f"Total Neto: ${fin['total']:,.0f} COP")

            # 5. NOTAS LEGALES Y ADVERTENCIAS
            st.caption("---")
            if "FACTURA" in tipo_doc.upper():
                st.caption(
                    "*Este documento cumple con el formato UBL 2.1 exigido por la DIAN.*"
                )
            else:
                st.caption(
                    "*Documento informativo para fines comerciales. No es una factura de venta.*"
                )

            st.warning(
                "**Verificación:** Revisa que el NIT y el Email sean correctos para la firma electrónica."
            )
        else:
            st.error(
                "El carrito está vacío. Agrega productos para ver la previsualización."
            )
        ...
        try:
            ...
            if "FACTURA" in tipo_doc.upper():
                with st.spinner("Legalizando documento ante la DIAN..."):
                    ...
                    # Usamos un contenedor de estado para informar al usuario
                    with st.status(
                        "📡 Comunicando con Proveedor Tecnológico...", expanded=True
                    ) as status:
                        st.write("Generando JSON estructurado...")
                        from modules.siigo_adapter import SiigoAdapter

                        adapter = SiigoAdapter()

                        st.write("Enviando datos a la DIAN para validación...")
                        exito_api, msg_api = adapter.enviar_a_siigo(num_base)

                        if exito_api:
                            status.update(
                                label="✅ Validación DIAN Exitosa",
                                state="complete",
                                expanded=False,
                            )
                            st.toast(f"Documento legalizado: {msg_api}")
                        else:
                            status.update(
                                label="❌ Error en Validación Electrónica",
                                state="error",
                                expanded=True,
                            )
                            st.error(f"Detalle: {msg_api}")
            ...
            # 5. Registro en Base de Datos y Gestión de Microservicios
            if os.path.exists(archivo_pdf):
                if tipo_doc == "FACTURA DE VENTA":
                    # --- PREPARACIÓN DE COSTOS (Local) ---
                    carrito_con_costos = []
                    for item in st.session_state.carrito:
                        prod_info = st.session_state.db.obtener_producto_por_codigo(
                            item["cod"]
                        )
                        costo_real = (
                            prod_info["costo_proveedor"]
                            if prod_info and "costo_proveedor" in prod_info
                            else 0.0
                        )
                    ...
                    # --- 🚀 MEJORA: MICROSERVICIO DE INVENTARIO ---
                    from modules.inventory_adapter import InventoryAdapter

                    inv_adapter = InventoryAdapter()
                    # Delegamos la responsabilidad de stock al adaptador
                    exito_inv, msg_inv = inv_adapter.procesar_descuento_stock(
                        st.session_state.carrito
                    )

                    if not exito_inv:
                        st.error(f"❌ Fallo de Inventario: {msg_inv}")
                        return  # Detenemos todo si el inventario falla
                    ...
                    # --- 🚀 MEJORA: MICROSERVICIO DE CONTABILIDAD (IVA Y AUDITORÍA) ---
                    from modules.accounting_service import AccountingService

                    acc_service = AccountingService()
                    acc_service.registrar_impuestos(num_base, fin)

                    # --- 🚀 MEJORA: MICROSERVICIO DE FACTURACIÓN (DIAN) ---
                    from modules.siigo_adapter import SiigoAdapter

                    with st.spinner(
                        "🚀 Microservicio: Generando reporte para la DIAN..."
                    ):
                        adapter = SiigoAdapter()
                        exito_api, msg_api = adapter.enviar_a_siigo(num_base)
                        if exito_api:
                            st.toast("✅ Reporte de Facturación Electrónica preparado")
                        else:
                            st.error(
                                f"⚠️ Error en el adaptador de facturación: {msg_api}"
                            )
                    # --- FIN DE LA MEJORA ---

                    # --- 🚀 MEJORA VISUAL: PROCESAMIENTO API CON STATUS ---
                    with st.status(
                        "📡 Comunicando con Proveedor Tecnológico...", expanded=True
                    ) as status:
                        st.write("Generando JSON estructurado para DIAN...")
                        from modules.siigo_adapter import SiigoAdapter

                        adapter = SiigoAdapter()

                        st.write("Enviando datos y validando firma digital...")
                        exito_api, msg_api = adapter.enviar_a_siigo(num_base)
                    ...
                    # Registro para Cotizaciones (No se envían a la DIAN)
                    st.session_state.db.registrar_venta(
                        fecha=str(fecha),
                        tipo=tipo_doc,
                        num=num_base,
                ...
                st.session_state.carrito = (
                    []
                )  # Limpiar para la siguiente venta# Limpiar para la siguiente venta
            else:
