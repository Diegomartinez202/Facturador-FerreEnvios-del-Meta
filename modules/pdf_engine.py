...
class PDFEngine:
    ...
    def generar_documento(self, tipo, num_doc, fecha, cliente_data, items_carrito, info_financiera, cufe=None):
        ...
        doc = SimpleDocTemplate(nombre_archivo, pagesize=letter, rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=50)
        story = []
        ...
        style_azul = ParagraphStyle(
            'Azul', parent=style_normal, fontSize=11, fontName='Helvetica-Bold', 
            textColor=colors.HexColor("#1A365D"), leading=14, spaceAfter=2
        )
        
        # 2. Estilos para el Encabezado (Logo y Empresa)
        style_empresa = ParagraphStyle('Emp', parent=style_normal, fontSize=10, leading=12, leftIndent=0, spaceBefore=0, spaceAfter=0)
        
        # 3. Estilos para la información del Documento (No. y Fecha)
        style_titulo = ParagraphStyle('T_Gen', parent=style_normal, fontName='Helvetica-Bold', fontSize=18, textColor=colors.HexColor("#1A365D"), leading=20)
        style_titulo_doc = ParagraphStyle('T_Doc', parent=style_normal, fontName='Helvetica-Bold', fontSize=14, leading=20, alignment=2)
        style_doc_derecha = ParagraphStyle('DocDer', parent=style_normal, fontName='Helvetica-Bold', fontSize=14, leading=18, alignment=2)

        # 4. Estilos para el Cliente
        style_cliente_derecha = ParagraphStyle('CliDer', parent=style_normal, fontSize=10, leading=14, alignment=2)
        style_cliente_lista = ParagraphStyle('Cli', parent=style_normal, fontSize=10, leading=14, alignment=0)
        ...
        # Título idóneo DIAN
        titulo_display = "DOCUMENTO EQUIVALENTE A FACTURA DE VENTA" if "FACTURA" in tipo.upper() else tipo
...
# CREACIÓN DE LA TABLA (Definición obligatoria antes de usarla)
        ...
        cliente_table_data = [[Paragraph(col_cliente_1, style_cliente_lista), 
        ...
        if v_flete > 0:
            tabla_data.append(["", "", "", "", Paragraph("<b>FLETE:</b>", style_sub), Paragraph(f"${v_flete:,.0f}", style_sub)])
        ...
        i_table.setStyle(TableStyle([
            ...
            ('ALIGN', (3,0), (-1,-1), 'RIGHT'), # Cantidad y precios a la derecha
...
# --- 5. PIE DE PÁGINA DINÁMICO (VERSIÓN ÚNICA E IDÓNEA) ---
        ...
        if cufe and "FACTURA" in tipo.upper():
            story.append(Spacer(1, 15))
            path_qr = LegalLogic.generar_qr_dian(tipo, num_doc, cufe, info_financiera['total'])
            ...
            dian_table = Table([[img_qr, Paragraph(texto_dian, style_cufe)]], colWidths=[80, 370])
            dian_table.setStyle(TableStyle([
        ...
        # CRÉDITOS FINALES
        story.append(Spacer(1, 10))
        story.append(Paragraph("Software de Facturación: FerreEnvios v2.0 - Desarrollador: Diego Martínez", style_legal))
