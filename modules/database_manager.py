...
class DatabaseManager:
    ...
    def inicializar_tablas(self):
        ...
        # 5. Nueva Tabla de Configuración (Dinámica)
        cursor.execute('''
        ...
        try:
            # Costos y Tarifa IVA
            cursor.execute("ALTER TABLE productos ADD COLUMN costo_proveedor REAL DEFAULT 0.0")
            cursor.execute("ALTER TABLE productos ADD COLUMN tarifa_iva REAL DEFAULT 19.0")
            ...
            cursor.execute("ALTER TABLE historial_ventas ADD COLUMN costo_flete REAL DEFAULT 0.0")
            cursor.execute("ALTER TABLE historial_ventas ADD COLUMN estado_dian TEXT DEFAULT 'PENDIENTE'")
            
            # Códigos internacionales UNSPSC y Unidad de Medida
            cursor.execute("ALTER TABLE productos ADD COLUMN codigo_unspsc TEXT DEFAULT '27110000'")
            cursor.execute("ALTER TABLE productos ADD COLUMN unidad_medida TEXT DEFAULT '94'")
        except sqlite3.OperationalError:
            pass 
    ...
    def registrar_venta(self, fecha, tipo, num, cliente, total, carrito=None, cufe=None, xml=None, flete=0.0):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (fecha, tipo, num, cliente, total, cufe, xml, flete, 'EXITOSO' if cufe else 'LOCAL'))
    ...
    def obtener_siguiente_numero(self, tipo):
        conn = self.conectar()
        cursor = conn.cursor()
        # Buscamos el número máximo. Ignoramos letras con CAST
        cursor.execute("SELECT MAX(CAST(numero_doc AS INTEGER)) FROM historial_ventas WHERE tipo_doc = ?", (tipo,))
        res = cursor.fetchone()[0]
        conn.close()
        return (res + 1) if res else 1000
    ...
    def importar_desde_excel(self, df):
        ...
        for _, fila in df.iterrows():
            try:
                ...
                cat = str(fila.get('Categoría', fila.get('Categoria', 'General'))).strip()
                if cat == 'nan' or cat == '': cat = 'General'
    ...
    def insert_producto(codigo, descripcion, precio):
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO productos (codigo, descripcion, precio) VALUES (?, ?, ?)", 
